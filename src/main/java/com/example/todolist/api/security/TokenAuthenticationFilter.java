...
public class TokenAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
  ...
  public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
    ...
    AuthUser authUser;
    try {
      authUser = jwtUtils.parseToken(token);
      return new JwtAuthenticationToken(authUser, authUser.getAuthorities());

    } catch (AuthTokenExpiredException | AuthTokenInvalidException e) {
      return handleError(request, response, token);
    }
  }

  private Authentication handleError(HttpServletRequest request, HttpServletResponse response, String token) {
